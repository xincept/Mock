(function (global, factory) {
	...
	(global.Mock = global.Mock || {}, global.Mock.js = factory());
	...
	*/
	...
	var DICT_FIXED = (function() {
	  var fixed = [];
	  for (var id in DICT$1) {
	    var pid =
	      id.slice(2, 6) === '0000' ? undefined : id.slice(4, 6) == '00' ? id.slice(0, 2) + '0000' : id.slice(0, 4) + '00';
	    fixed.push({
	      id: id,
	      pid: pid,
	      name: DICT$1[id],
	...
	*/
	...
	var address = {
	  ...
	  province: function() {
	    return this.pick(DICT_FIXED).name
	  },
	  // 随机生成一个（中国）市。
	  city: function(prefix) {
	    var province = this.pick(DICT_FIXED);
	...
	*/
	var misc = {
	  ...
	  id: function() {
	    ...
	    id = this.pick(DICT_FIXED).id + this.date('yyyyMMdd') + this.string('number', 3);
	...
	*/
	var Assert = {
	  message: function(item) {
	    return (
	      item.message ||
	      `[${item.type.toUpperCase()}] Expect ${(Util.isArray(item.path) && item.path.join('.')) ||
        item.path}'${item.type.toLowerCase()} ${item.action} ${item.expected}, but is ${item.actual}`
	    )
	  },
	...
	*/
	...
	var XHR = new Proxy(MockXMLHttpRequest, {
